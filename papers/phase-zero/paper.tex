...
Currently we are looking at using Riak \cite{riak} for our database engine.
Riak has the unique ability to perform automatic masterless replication and
syncrhonization between all nodes of a database cluster. With this architecture 
all backend nodes except for one could fail and our system would remain
operational.
\begin{center}
